...
from src.repositories import user_repository, token_repository
...
class UserServie:
    ...
    async def refresh_token(
        ...
        return await token_repository.refresh_token(session=session, token_data=token_data, payload=payload)
    ...
    async def logout(
        ...
        token = await token_repository.check_token_validity(session=session, access_token=token, expired=False)
        await token_repository.update_token(session=session, expired=True, token_id=token.id)
    ...
    async def reset_password(
        ...
        await user_repository.update_data(
            session=session,
            data=dict(otp_code=generate_otp_code()),
            user_id=user.id
        )
    ...
    async def set_new_password(
        ...
        await user_repository.update_data(session=session, user_id=user.id, data=dict(
            password=hash_password(payload.new_password)
        ))
